...
create table T_CUENTABANCARIA (disc varchar(31) not null, id bigint not null, descubiertoAcordado double not null, fechaCierre date, fechaCreacion date not null, nroCuenta bigint not null, saldoActual double not null, saldoInicial double not null, titular_id bigint not null, primary key (id));
...
create table T_MOVIMIENTO (disc varchar(31) not null, id bigint not null, descripcion varchar(255) not null, fechaHora timestamp not null, monto double not null, primary key (id));
